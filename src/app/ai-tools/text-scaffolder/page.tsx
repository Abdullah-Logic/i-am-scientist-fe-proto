...
export default function TextScaffolder() {
    const [isEditorOpen, setIsEditorOpen] = useState(true);
    const [gradeLevel, setGradeLevel] = useState("9th grade");
    const [numQuestions, setNumQuestions] = useState("3");
    const [maxVocab, setMaxVocab] = useState("5");
    ...
    const handleGenerate = async () => {
        if (!originalText) {
            alert("Please provide the original text to scaffold.");
            return;
        }
        ...
        try {
            const response = await fetch("https://api.iamscientist.ai/api/text_scaffolder_v2/generate", {
                method: "POST",
                headers: {
            ...
            if (data.success) {
                setScaffoldResult(data.textScaffoldData);
                setIsEditorOpen(false); // Close editor to show the support wrapper
            } else {
                alert("Error: " + data.error);
            }
        } catch (error) {
            console.error("Scaffolding Linking Error:", error);
        ...
        Back
                  ...
                  {/* Breadcrumb */}
                    <div className="ts-breadcrumb">
                        ...
                        <span className="ts-breadcrumb-current">Text Scaffolder</span>
                    </div>
<div className="ts-container">
                    ...
                    <div className="ts-panel">
                        ...
                        {isEditorOpen && (
                            <div className="ts-form">
                                ...
                                <div className="ts-title-row">
                                    <h1 className="ts-title">Text Scaffolder</h1>
                                    <button className="ts-exemplar-btn">
                                ...
                                <p className="ts-subtitle">Scaffold text for readers who are behind grade level or need extra support.</p>
                                ...
                                <div className="ts-field">
                                    <label className="ts-label">
                                        Number of Literal Questions to Ask: <span className="ts-required">*</span>
                                    </label>
                                    <div className="ts-select-wrapper">
                                        <select
                                            className="ts-select"
                                ...
                                {/* Maximum Vocabulary Words */}
                                <div className="ts-field">
                                    <label className="ts-label">
                                        Maximum Vocabulary Words to Define: <span className="ts-required">*</span>
                                    </label>
                                    <div className="ts-select-wrapper">
                                        <select
                                            className="ts-select"
                                ...
                                <div className="ts-field" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                    <label className="ts-label">
                                        Original Text: <span className="ts-required">*</span>
                                    </label>

                                    <div className="ts-textarea-outer" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: '220px' }}>
                                        ...
                                        <div className="ts-field-footer-inner">
                                            ...
                                            <span className="ts-word-count">Total word limit: {wordCount(originalText)}/75,000</span>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {/* --- NEW SCAFFOLD OUTPUT VIEW --- */}
                        {!isEditorOpen && scaffoldResult && (
                            <div className="ts-output-area" style={{ padding: '20px', color: 'white', borderTop: '1px solid rgba(135,45,230,0.3)', overflowY: 'auto' }}>
                                <h2 style={{ fontSize: '1.2rem', marginBottom: '1rem', color: '#a855f7' }}>Reading Support Wrapper:</h2>
                                ...
                                <button className="ts-generate-btn" style={{ marginTop: '20px' }} onClick={() => setIsEditorOpen(true)}>Back to Parameters</button>
                            </div>
                        )}
    ...
    gap: 0.5rem;
                ...
                .ts-exemplar-btn:hover { color: #c084fc; text-decoration: underline; }
